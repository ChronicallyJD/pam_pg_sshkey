...
- The `source-address` critical option is enforced instead of refused. A
  ...
  with `log_hostname = on`, where the item can be a name; a malformed list,
  including a stray comma, refuses rather than matching what it can.
  ...
  `test_source_address_list_and_ipv6`, and
