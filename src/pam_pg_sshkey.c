 ...
 *      timestamp and a nonce the client issued, with a base64-encoded
 *      Ed25519 or RSA signature over them, made by its SSH private key.
 ...
 *
 *     <unix_ts>:<nonce_hex>:<base64_signature>[:<base64_certificate>]
 *
 * The client issues the timestamp and the nonce itself.  This module bounds
 * the timestamp, verifies the signature, and records the nonce so the token
 * cannot be used twice (see challenge_store.[ch]).
 *
                    ...
                    int argc, const char **argv)
{
    ...
    /*
     * v2  "<ts>:<nonce_hex>:<sig>"          two colons
     * v3  "<ts>:<nonce_hex>:<sig>:<cert>"   three colons, certificate
     *
     * Anything else, including the retired v1 shape "<nonce_hex>:<sig>",
     * is malformed.
     */
