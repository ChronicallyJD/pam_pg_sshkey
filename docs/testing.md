...
| `tests/test_ssh_agent` | 6 | `ssh_agent.c` against a hostile agent served by the test: a silent agent times out, oversize and truncated replies, an inner string that overruns, an empty signature (with a certificate blob, where the local check is skipped), terminal escapes in the algorithm name, a refusal followed by a close (which must not raise `SIGPIPE`), a signature by another key, and a genuine signature under a wrong algorithm name, all under AddressSanitizer |
