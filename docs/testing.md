...
| --- | --- | --- |
| `tests/test_challenge_store` | 7 | Recording a nonce once and refusing it thereafter, the record's mode, names that are not 64 lowercase hex characters, and the sweep of records too old to matter |
...
| `tests/test_integration` | 4 | The library flow behind a token: build the signed message, sign, parse `authorized_keys`, verify, record the nonce |
| `tests/test_pam_module` | 62 | The production `pam_pg_sshkey.so`, loaded through libpam with `pam_start_confdir` and a PostgreSQL-style conversation function: success, replay, a retired v1 token refused, expired and future timestamps, wrong key, tampered and malformed tokens, missing, empty, and group-writable key files, a private `0700` nonce directory, fail-closed recording, RSA, `acct_mgmt`, and stale-nonce sweeping; and, with `trusted_ca_keys=` on the service line, certificates: an Ed25519 certificate with no `authorized_keys`, RSA key under an Ed25519 CA, Ed25519 key under an RSA CA (`rsa-sha2-512` accepted, `ssh-rsa` refused), and refusal of an untrusted CA, expired, not yet valid, wrong principal, no principals, host certificate, critical option, `trusted_ca_keys` unset, group-writable CA file, replay, timestamps outside the window, a tampered certificate byte, a token signed by a key other than the certified one, a token of 8192 bytes or more, and a certificate whose key id or option name contains a newline, with v2 tokens still passing; and, against a real `ssh-agent` started by the test, `--agent`: an Ed25519 key deleted from disk after `ssh-add`, an RSA key (which proves the client asks for `rsa-sha2-256`), a passphrase-protected key that no other route can use, a certificate signed through the agent, an unregistered agent key refused, and no agent, a dead socket, and an agent without the key each producing no token. Each negative test asserts the return code, the log line, and that no nonce was recorded. Revocation is covered by a revoked key, a revoked certified key, a revoked security key, an unreadable list refusing every login, a group-writable list, and one revoked key not locking out another. Security keys are covered by a signature in the authenticator's format (built by `tests/sk_helper.py`, not by hardware): the user-presence bit required, the application and counter covered by the signature, and `sk-ecdsa` skipped as an unsupported type. |
| `tests/test_system` | 16 | The built `pg_sshkey_sign` binary as a user runs it, including `--cert`: a four-field token whose fourth field is the certificate file's base64, the v3 prefix under the signature, and exit 1 for a missing file, a plain public key, or an unpadded certificate field; and `--agent` refusing a missing `SSH_AUTH_SOCK`, an `sk-ecdsa` key type, a private key file, and a stray positional argument |
| `tests/test_python_module.py` | 84 | `pam_pg_sshkey.py`: key loading, signing, the token shape and prefix, error mapping, `connect_replication` selection, `cert_path`, `agent_pubkey` against a real `ssh-agent` and against a fake one (byte-identical to `pg_sshkey_sign`, a non-default application, no user presence refused, `sk-ecdsa` refused), and that the retired v1 parameters now raise `TypeError`. Four tests skip when `bcrypt` is absent |
| `tests/test_pg_sshkey_query.py` | 17 | `pg_sshkey_query` and `pg_sshkey_connect` as subprocesses: error handling, `PGDATABASE` and `PGPORT`, helper lookup, `--cert` and `--agent` reaching `pg_sshkey_sign`, `-i` with `--agent` refused, and `--v1` and the challenge options now rejected as unknown |
...
roles, and runs `tests/e2e/check.sh`. The 41 checks log in as real users
...
token, expired and future timestamps, a user without keys, a malformed
token, a nonce directory the module cannot write, a private nonce directory
no client can write to, a client under `umask 077`, a token of the retired v1 shape, and the retired `--v1`
option being refused. The certificate checks use a
