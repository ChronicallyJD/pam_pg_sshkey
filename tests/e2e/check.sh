...
restore_chal_dir() { chown postgres:postgres "$CHAL_DIR"; chmod 0700 "$CHAL_DIR"; }
...
# ── the nonce directory is private to postgres ──────────────────────────────
check_private_nonce_dir() {
    chmod 0700 "$CHAL_DIR"
    local rc=0 out
    # no client writes here any more: only the module records nonces
    if as_alice "touch $CHAL_DIR/probe" >/dev/null 2>&1; then echo "a user wrote to the nonce directory"; rc=1; fi
...
check_cert_alice_key_without_cert_still_works() {
    ...
    expect_journal "$cur" "pam_pg_sshkey: user 'alice' authenticated with key" || return 1
    # and the retired v1 option is gone from the client
    out=$(as_alice "pg_sshkey_connect --v1 -h 127.0.0.1 -- -tAc 'select 1'" 2>&1) \
        && { echo "--v1 was accepted"; return 1; }
    grep -q "Unknown option: --v1" <<<"$out" || { echo "unclear error: $out"; return 1; }
}
...
# ── the retired v1 token ────────────────────────────────────────────────────
check_v1_shaped_token_rejected() {
    # <nonce_hex>:<signature>, the shape the server accepted until 2.0.0
    local cur; cur=$(journal_mark)
    local out; out=$(as_alice "
        tok=\$(pg_sshkey_sign ~/.ssh/id_ed25519) || exit 9
        v1=\$(cut -d: -f2,3 <<<\"\$tok\")
        PGPASSWORD=\$v1 psql -h 127.0.0.1 -U alice -d alice -tAc 'select 1'
    " 2>&1) && { echo "a v1-shaped token authenticated"; return 1; }
    expect_journal "$cur" "pam_pg_sshkey: malformed token for 'alice'"
}
...
run_check v1_shaped_token_rejected
